...
TYPE part1_process(TYPE x) {
    ...
    static const TYPE v_bias_b = 0;
    static TYPE C_2_a = 0, C_2_b = 0;
    C_2_b = C_2_a;
    ...
    TYPE R_A_a = - inp_inner_ser_reflect * (inner_ser_a); // port1.b - reflect * (x + port1.b + port2.b) // ser(R_A, V_bias)
    TYPE v_bias_a = - (x + R_A_a); // b2 = - (x + b1)
    return (v_bias_a)/2; // voltage
}
...
// #define DEBUG_PRINT
    ...
    for (int t=0; t<N; t++) {
        ...
        TYPE x = sinf(2*3.14*1000*t/Fs); // 1000 Hz
#else
        TYPE x = input[t];
#endif
        clock_t t1 = clock();
    ...
    write_pcm("../output_nobias.pcm", output, N);
}
