...
hold on;
noise = plot(data.xaxis,noise_dB,'-','LineWidth',3);
title('RT20 Test For 4s Reverb Time', FontSize=14);
xlim([0.5 4]);
ylim([-40 5]);

xline(0.75, '--', '0.75s', 'LineWidth', 1.5, 'Color', 'r');
xline(1.8, '--', '1.8s', 'LineWidth', 1.5, 'Color', 'r');

yline(MaxVAL-5,'--', '-5dB', 'LineWidth',1.5,'color','black');
yline(MaxVAL-25,'--', '-25dB', 'LineWidth',1.5,'color','black');

legend([s2, s1, noise], {'Output', 'Input', 'Noise Floor'}, 'Location', 'SouthWest', FontSize=12);
xlabel('Time (seconds)', FontSize=14);
ylabel('Magnitude (dB)', FontSize=14);

saveFolder = fullfile(pwd,'..','..','..','gitfigures/system_design/reverb/');
filePath = fullfile(saveFolder, sprintf('%s.png','reverb_rt60_test_noisefloor'));
exportgraphics(figc(1), filePath, 'Resolution', 300);
