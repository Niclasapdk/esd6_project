...
early_offset = 0;
early_delayline = zeros(early_M,1); 
...
%section
late_M = max(tap_delay_samples);
output_delayline = zeros(late_M,1);
late_offset = 0;
%delay late reverb
for n = 1:length(input)
    i = ceil(max(tap_delay_samples)/3);
    late_decay_out(n) = output_delayline(ringbuf_idx(late_offset,i,late_M));
    late_offset = mod(late_offset-1,late_M);
    output_delayline(late_offset+1) = allpass_out(n);
end
