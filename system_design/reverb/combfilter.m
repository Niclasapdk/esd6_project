...
%y[n]=x[n-m]-g1*x[n-m-1]+g1*y[n-1]+g2*y[n-m]   with
%rememberinput delayline n-m-1 m+1 length
%y[n] = x[n] - reverbG1 * x[n-1] + reverbG1 * y[n-1] + reverbG2 * y[n-m]
%without delayline
for n=1:input_length
    output(n)=input(n)-gain_LP*input_delay+gain_LP*output_delayline(ringbuf_idx(start_output_offset, 1, M))...
        +g2*output_delayline(ringbuf_idx(start_output_offset, M, M));%-1 pga matlab
