...
folderPath = pwd;
files = dir(fullfile(folderPath, 'lfo_flanger_1hz_test.txt'));
data = cell(length(files), 1);
filePath = fullfile(folderPath, files.name);

x = readtable(filePath);
v = (1:6453)/4410; %44100/10 = 4410 :))
...
hold on
legend(p1, 'Time period = 0.998s','Location','southwest');
xlim([0 1.1]);
ylim([-1.1 1.1]);
grid on

saveFolder = fullfile(pwd,'..','..','gitfigures/system_design/ChorusFlanger');
filePath = fullfile(saveFolder, sprintf('%s.png', 'flanger_lfo_test_1000mHz'));
